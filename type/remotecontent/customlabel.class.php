...
class customlabel_type_remotecontent extends customlabel_type {
    ...
    protected function get_moodlews_content() {
        ...
        if ($this->data->wsprotocol == 'rest') {
            ...
            $serviceurl = $this->data->baseurl.'/webservice/rest/server.php';
            ...
            $result = $client->__soapCall($this->data->wsfunction, $paramarr);
        }
        ...
        if (!empty($this->data->mustache)) {
            ...
            foreach($data as $datum) {
                ...
                $template = $mustache->loadTemplate($this->data->mustache);
    ...
    protected function add_context(&$url) {
        ...
        if ($this->data->sendcontext == self::CONTEXT_SEND_PLAIN) {
            if (strpos($url, '?') !== false) {
                $url .= '&'.$query;
            } else {
                $url .= '?'.$query;
