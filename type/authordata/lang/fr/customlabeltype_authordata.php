<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * This is a local class contextual translation file for field names and list options.
 * this file is automatically loaded by the /mod/customlabel/lang/xx_utf8/customlabel.php
 * module language file.
 *
 * @package    mod_customlabel
 * @category   mod
 * @author     Valery Fremaux <valery.fremaux@gmail.com>
 * @copyright  (C) 2008 onwards Valery Fremaux (http://www.mylearningfactory.com)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL
 */

// Privacy.
$string['privacy:metadata'] = 'Le composant d\'élément de cours AuthorData ne détient directement aucune donnée relative aux utilisateurs.';

$string['authordata:view'] = 'Peut voir le contenu';
$string['authordata:addinstance'] = 'Peut ajouter une instance';

$string['pluginname'] = 'Elément de cours : Information sur les auteurs';
$string['typename'] = 'Information sur les auteurs';
$string['configtypename'] = 'Activer le type Information sur les auteurs';
$string['author1'] = 'Auteur 1';
$string['author2'] = 'Auteur 2';
$string['author3'] = 'Auteur 3';
$string['author4'] = 'Auteur 4';
$string['author5'] = 'Auteur 5';
$string['institution1'] = 'Institution 1';
$string['institution2'] = 'Institution 2';
$string['institution3'] = 'Institution 3';
$string['institution4'] = 'Institution 4';
$string['institution5'] = 'Institution 5';
$string['department1'] = 'Département 1';
$string['department2'] = 'Département 2';
$string['department3'] = 'Département 3';
$string['department4'] = 'Département 4';
$string['department5'] = 'Département 5';
$string['thumb1'] = 'Vignette 1';
$string['thumb2'] = 'Vignette 2';
$string['thumb3'] = 'Vignette 3';
$string['thumb4'] = 'Vignette 4';
$string['thumb5'] = 'Vignette 5';
$string['role1'] = 'Rôle 1';
$string['role2'] = 'Rôle 2';
$string['role3'] = 'Rôle 3';
$string['role4'] = 'Rôle 4';
$string['role5'] = 'Rôle 5';
$string['tablecaption'] = 'Titre de table';
$string['contributors'] = 'Contributeurs (autres)';
$string['institution'] = 'Institution';
$string['department'] = 'Département';
$string['commonswitches'] = 'Commutateurs d\'affichage';
$string['showcontributors'] = 'Afficher les contributeurs';
$string['showinstitution'] = 'Afficher l\'institution';
$string['showdepartment'] = 'Afficher le département';
$string['showrole'] = 'Afficher le rôle';
$string['leftcolumnratio'] = 'Largeur colonne gauche';

$string['author'] = 'Auteur';
$string['topicexpert'] = 'Expert thématique';
$string['reviewer'] = 'Relecteur';
$string['illustrator'] = 'Illustrateur';
$string['pedagogicdesigner'] = 'Designer pédagogique';
$string['assessmentdesigner'] = 'Designer des évaluations';

