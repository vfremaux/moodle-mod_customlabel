...
$classes = customlabel_get_classes(null, true);
$roles = get_records_menu ('role', '', '', 'name', 'id,name') ;

$settings->add(new admin_setting_heading('storage', get_string('storage', 'customlabel'), ''));

$settings->add(new admin_setting_configcheckbox('usesafestorage', get_string('usesafestorage', 'customlabel'), get_string('configusesafestorage', 'customlabel'), 1));
...
foreach($classes as $class){
    ...
    $description = get_string('enabletype', 'customlabel') . ' ' . $class->name;
	$settings->add(new admin_setting_configcheckbox("list_{$parmname}", $class->name, $description, 1));
    $description = get_string('hiddenrolesfor', 'customlabel') . ' ' . $class->name;
